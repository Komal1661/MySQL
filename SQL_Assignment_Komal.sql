...
#1.a.	Fetch the employee number, first name and last name of those employees who are working as Sales Rep reporting to employee with employeenumber 1102 
...
#3.a.	Using the OrderDetails table, identify the top 10 products (by productCode) with the highest total order quantity across all orders.
...
#4. Create a new database named and Customers_Orders and add the following tables 
...
#5.a. List the top 5 countries (by order count) that Classic Models ships to. (Use the Customers and Orders tables)
...
CALL InsertIntoEmp_EH(1, 'B Komal', 'salvek34@gmail.com');
